...
class nb6run:
    ...
    def __init__(self, PATH):
        ...
        self.snapshots_out9(n, time_arr, headers, E_ks, ks_bindata_snp,
                            Npair_arr)
        del ks_bindata_snp
    ...
    def snapshots_out3(self, n, headers, Ms, Xs, Vs, Ns):
        ...
        for i in range(0, n):
            ...
            snap_array.append(
                snapshot(time, headers[i], Ms[i], X_arr, V_arr, Ns[i]))
        return snap_array

    def snapshots_out9(self, n, time_arr, headers, E_ks, ks_bindata_snp,
                       Npair_arr):
        """snapshots_out9.
        ...
        headers :
            headers of OUT9 file
        E_ks :
            E_ks
        ks_bindata_snp :
            Table of each KS binary in each snapshot
        Npair_arr :
            Number of pairs in each snapshot
        """
        for i in range(0, n):
            if time_arr[i] * self.snaps[i].o3_header[4][10] == self.snaps[
                    i].time:
    ...
    def read_out9(self):
        ...
        with open(filename, "r") as f:
            ...
            while True:
                ...
                TIME = h1[0][6]
                header = list(h1[0])
                h2 = ascii.read(f.readline(),
                                format='no_header',
                                guess=False,
                                fast_reader=True)
                E_k = list(h2[0])
                h3 = ascii.read(f.readline(),
                                format='no_header',
                                guess=False,
                                fast_reader=True)
                ...
                ks_bindata = ascii.read(
                    temp,
                    format='no_header',
                    guess=False,
                    fast_reader=True,
                    names=('E_b', 'ecc', 'E_cm', 'r', 'm1', 'm2', 'P', 'name1',
                           'name2', 'Stype1', 'Stype2', 'merge_type'))
                n = n + 1
